...
CASAuthentication.prototype._login = function(req, res, next) {
    ...
    console.log('go to cas');

    // Set up the query parameters. The server block the access if service_url path is not url
    this.service_url = req.body.url;
    //this.service_url = "http://10.82.124.76/mmrt/";
    var query = {
        service: this.service_url/* + url.parse(req.url).pathname,
        renew: this.renew*/
    };
    delete req.body.user.pass;
    var request = {
        'url' : this.cas_url + url.format({
            pathname: '/login',
            query: query
        }),
        'user' : req.body.user
